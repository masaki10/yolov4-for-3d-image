...
def YOLOv4(input_layer, NUM_CLASS):
    ...
    route = conv
    conv = common.convolutional(conv, (1, 1, 1, 512, 256))
    conv = common.upsample(conv)
    route_2 = common.convolutional(route_2, (1, 1, 1, 512, 256))
    ...
    route_2 = conv
    conv = common.convolutional(conv, (1, 1, 1, 256, 128))
    conv = common.upsample(conv)
    route_1 = common.convolutional(route_1, (1, 1, 1, 256, 128))
...
def decode_train(conv_output, output_size, NUM_CLASS, STRIDES, ANCHORS, i=0, XYSCALE=[1, 1, 1]):
    conv_output = tf.reshape(conv_output,
                             (tf.shape(conv_output)[0], output_size, output_size, output_size, 3, 7 + NUM_CLASS)) #[x, y, z, w, h, d, conf]

    conv_raw_dxdydz, conv_raw_dwdhdd, conv_raw_conf, conv_raw_prob = tf.split(conv_output, (3, 3, 1, NUM_CLASS), axis=-1)

    xyz_grid = tf.meshgrid(tf.range(output_size), tf.range(output_size), tf.range(output_size))
    xyz_grid = tf.expand_dims(tf.stack(xyz_grid, axis=-1), axis=3)  # [gx, gy, gz, 1, 3]
    xyz_grid = tf.tile(tf.expand_dims(xyz_grid, axis=0), [tf.shape(conv_output)[0], 1, 1, 1, 3, 1]) # [1, gx, gy, gz, 1, 3] -> [N, gx, gy, gz, 3, 1]

    xyz_grid = tf.cast(xyz_grid, tf.float32)

    pred_xyz = ((tf.sigmoid(conv_raw_dxdydz) * XYSCALE[i]) - 0.5 * (XYSCALE[i] - 1) + xyz_grid) * STRIDES[i]
    pred_whd = (tf.exp(conv_raw_dwdhdd) * ANCHORS[i])
    pred_xyzwhd = tf.concat([pred_xyz, pred_whd], axis=-1)

    pred_conf = tf.sigmoid(conv_raw_conf)
    pred_prob = tf.sigmoid(conv_raw_prob)

    return tf.concat([pred_xyzwhd, pred_conf, pred_prob], axis=-1)
...
def compute_loss(pred, conv, label, bboxes, STRIDES, NUM_CLASS, IOU_LOSS_THRESH, i=0):
    ...
    conv = tf.reshape(conv, (batch_size, output_size, output_size, output_size, 3, 7 + NUM_CLASS))
    ...
    bbox_loss_scale = 2.0 - 1.0 * label_xywh[:, :, :, :, :, 3:4] * label_xywh[:, :, :, :, :, 4:5] * label_xywh[:, :, :, :, :, 5:6] / (input_size ** 3)
    giou_loss = respond_bbox * bbox_loss_scale * (1- giou)

    iou = utils.bbox_iou(pred_xywh[:, :, :, :, :, np.newaxis, :], bboxes[:, np.newaxis, np.newaxis, np.newaxis, np.newaxis, :, :])
